...
@Test func retryingAgentMaxRetriesValidation() {
    #expect(throws: AgentConfigurationError.self) {
        _ = try AgentConfiguration(serverURL: "http://127.0.0.1:1234/v1/responses", modelName: "test-model", maxRetries: 0)
    }
    #expect(throws: AgentConfigurationError.self) {
        _ = try AgentConfiguration(serverURL: "http://127.0.0.1:1234/v1/responses", modelName: "test-model", maxRetries: 11)
    }
}
...
@Test func retryingAgentNonRetryableErrorPropagatesImmediately() async throws {
    ...
    let config = try AgentConfiguration(serverURL: "http://127.0.0.1:19999/v1/responses", modelName: "test-model", maxRetries: 3)
    let agent = try RetryingLLMChatAgent(configuration: config)
    do {
